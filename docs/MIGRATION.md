...
2. Start new missions from `config.release.yaml`. In the public source, `config.yaml` and `config.release.yaml` use the same portable defaults. If upgrading an existing installation, keep your operator settings in a separate local configuration and explicitly preserve the old mission namespace only when continuing that mission.
