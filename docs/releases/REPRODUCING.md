# Reproducing the release results

The curated full source archive is the reproducibility entry point. It contains the AMY runtime, Atlas source, the two calibration harnesses, English manuals and numerical evidence. The Python wheel is a runtime component, not the complete laboratory. This source snapshot excludes Git history, credentials, local state and raw private conversations.
