...
async def generate_discussion_llm(
    ...
    discussion) when there are no usable results to ground on. Explicitly
    unfinished or token-limited responses are retried once, then rejected;
    legacy clients that omit completion metadata remain supported.
    """
    ...
    for attempt in range(2):
        try:
            ...
            # A long fragment can pass every prose/grounding check after its
            # process preamble is removed. Completion metadata must therefore
            # be checked before reading either content or the thinking fallback.
            # Ollama's done flag indicates whether this chat response finished;
            # done=True with reason=length still means its token budget ran out.
            if resp.get("done_reason") == "length" or resp.get("done") is False:
                log.warning(
                    "llm_enhancer.incomplete_response",
                    model=model,
                    done=resp.get("done"),
