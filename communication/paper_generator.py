...
class PaperGenerator:
    ...
    def _build_markdown(
        ...
        if experiment_ids:
            ...
            for eid in experiment_ids:
                ...
                if output_hash:
                    valid_hashes.append(output_hash)
                    lines.append(
            ...
            if valid_hashes:
                ...
                evidence_tree = MerkleTree(valid_hashes)
                lines.append("")
                lines.append(f"**Evidence Merkle Root (RFC 6962):** `{evidence_tree.root}`")
            lines.append("")
    ...
    async def generate_from_llm(
        ...
        try:
            from cognition.reasoning import _clean_json, _extract_content, _parse_json_robust
            ...
            if not isinstance(data, dict) or "title" not in data or "sections" not in data:
                log.warning("paper_generator.llm_malformed_json", raw_snippet=raw[:300])
                title_match = re.search(r'"title"\s*:\s*"([^"]+)"', raw)
                title_val = title_match.group(1) if title_match else topic.strip()
                abstract_match = re.search(r'"abstract"\s*:\s*"([^"]+)"', raw, re.DOTALL)
                abstract_val = abstract_match.group(1) if abstract_match else (
                    f"Computational research study investigating {safe_topic} through rigorous empirical validation."
                )
                if not isinstance(data, dict):
                    data = {}
                data["title"] = data.get("title") or title_val
                data["abstract"] = data.get("abstract") or abstract_val
                if "sections" not in data or not isinstance(data["sections"], list) or not data["sections"]:
                    log.info("paper_generator.falling_back_to_structured", topic=topic)
                    return await self._generate_from_structured_fallback(
                        topic=topic,
                        knowledge_facts=knowledge_facts,
                        recent_thoughts=recent_thoughts,
            ...
            return await self.generate_paper(
                title=data.get("title", topic.strip() or "A.M.Y Computational Research Report"),
                abstract=data.get("abstract", f"Computational analysis of {safe_topic}."),
                sections=sections,
            ...
            log.info("paper_generator.fallback_on_exception", topic=topic)
            return await self._generate_from_structured_fallback(
                topic=topic,
                knowledge_facts=knowledge_facts,
                recent_thoughts=recent_thoughts,
