...
| Evolution (self-retrain belief-weight update + meta-review feedback) | ✅ Functional | does not fine-tune a neural net on-device |
...
**Multi-domain coverage:** Atlas domains produce papers end-to-end, with provenance integrity verified by SHA-256. New-pipeline papers (LLM enhancer + evolution) score meaningfully higher than prior versions on the project's own rubric and win a blind head-to-head against them — see [Quality](#paper-quality-vs-prior-versions).
...
- **Real scientific tool calls.** A registry of scientific tools across 23 domains, of which **9 are tagged `evidence_grade='real_local'`** — genuine library calls whose numbers are reproducible: **SymPy**, **NumPy/SciPy**, **PySCF** (HF / DFT), **ASE** (atomistic), **AstroPy** (cosmology + units), **PyMatGen** (crystal structures), **RDKit** & **BioPython** (via the chemistry/biology services), **Z3** prover. The remainder are heuristic, tabulated, or demo tools — see [Honest scope](#honest-scope-what-is-real-vs-heuristic) for exactly which is which.
...
5. **LEARN** — `memory/episodic.py` records what happened. `memory/semantic.py` updates the knowledge graph. `memory/procedural.py` chunks any new successful skill into the Voyager-style skill library at `skills/library.py`. `evolution/curriculum.py` decides what to study next. `evolution/self_retrain.py` improves A.M.Y in two implemented ways: (a) a **belief-weight update** — it re-estimates each world-model belief's confidence from that belief's accumulated confirm/contradict evidence (its empirical reliability) and persists the new values to the knowledge graph, a genuine Bayesian parameter update on the model that exists; and (b) **meta-review feedback propagation** via `cognition/meta_review_agent.py`, the Google Co-Scientist Meta-review mechanism that synthesizes recurring weaknesses from prior reviews and feeds them into the next cycle's prompts. (The Co-Scientist paper notes this kind of feedback loop "enables learning without back-propagation"; A.M.Y does **not** fine-tune a large neural network on-device.)
...
The registry counts ~94 callable tools at runtime once dynamic registration (`_register_service_tools`, `evidence_corroborate_<domain>`) is included. Of these, **9 are tagged `evidence_grade='real_local'`** (genuine, reproducible library calls); the rest are heuristic, tabulated, or demo — verify with `scripts/diagnostics/test_all_94_tools.py`. The counts below are the static registry by domain.
...
- **Blind head-to-head.** New papers beat prior-version papers **10/12** under an independent LLM judge (95% CI lower bound > 0.5; the two losses were against the system's own curated flagships).
...
4. **Learns for real.** It does not only accumulate text. It re-estimates world-model belief confidences from accumulated evidence (a real parameter update, persisted to the knowledge graph) and propagates synthesized review feedback into later cycles (the Co-Scientist Meta-review loop), so subsequent papers avoid earlier mistakes. It does not claim on-device fine-tuning of a large neural network.
...
- **Ranking, Reflection, Meta-review, and Evolution agents.** Implemented and tested (Elo tournament, initial rating 1200; six-check reflection rubric; recurring-weakness synthesis; Co-Scientist Evolution agent with the paper's five refinement strategies).
- **Belief-weight update.** `self_retrain` re-estimates world-model belief confidences from accumulated evidence and persists them — a real parameter update on the model that exists.
