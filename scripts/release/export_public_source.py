...
ROOT_FILES={'amy.py','pyproject.toml','uv.lock','requirements.txt','LICENSE','CITATION.cff','README.md','README_PUBLIC.md','SCIENCE_MANIFESTO.md','ATLAS_TOOL_GUIDE.md','RESEARCH.md','CONTRIBUTING.md','ENVIRONMENT.md','SECURITY.md','USE_POLICY.md','CHANGELOG.md','MANIFEST.in','pytest.ini','conftest.py','.env.example','config.release.yaml','.gitignore'}
...
POLICY=json.loads((ROOT/'scripts/release/public_content.json').read_text())

def candidates():
    selected={ROOT/n for n in ROOT_FILES if (ROOT/n).is_file()}
    for rel in PACKAGES+LABS+('atlas/app',):
        for p in (ROOT/rel).rglob('*'):
            if p.is_file() and not p.is_symlink() and p.suffix in {'.py','.md','.json','.yaml','.yml','.txt','.bib'} and not any(x in EXCLUDED or x.startswith('.') for x in p.relative_to(ROOT/rel).parts) and not p.relative_to(ROOT).as_posix().startswith('sandbox/scripts/'):selected.add(p)
    for rel in ('release_evidence',):
        for p in (ROOT/rel).rglob('*'):
            if p.is_file() and not p.is_symlink() and '__pycache__' not in p.parts:selected.add(p)
    for rel in tuple(POLICY.get('retained_source_files', ())) + tuple(POLICY['public_documents']) + tuple(POLICY['english_manuals']) + ('docs/en/TRANSLATION_MANIFEST.json','atlas/README.md','atlas/LICENSE','atlas/pyproject.toml','sandbox/Dockerfile','.github/workflows/release-hygiene.yml','.github/workflows/public-candidate.yml','atlas/kubernetes/secret.example.yml','scripts/run/run_receipt_memory_validation.py','scripts/run/run_native_science_benchmark.py','scripts/verify/package_native_science.py','scripts/verify/score_scientific_assessments.py','scripts/verify/summarize_native_benchmark.py','scripts/verify/verify_native_science_run.py','scripts/diagnostics/verify_secret_hygiene.py','scripts/diagnostics/verify_atlas_tools.py','scripts/diagnostics/verify_security_controls.py','atlas/.gitignore'):
        p=ROOT/rel
        if p.is_file():selected.add(p)
    selected.update(p for p in (ROOT/'atlas').glob('requirements*.txt') if p.is_file())
    selected.update(ROOT/'atlas'/name for name in ('main.py','main_refactored.py','run_agent_with_tools.py','setup.py'))
    # Local policy files required by the Atlas laboratory; environment credentials excluded.
    for rel in ('atlas/config/ethics_policy.yaml','atlas/config/agents.yaml','atlas/config/models.yaml','atlas/config/prompts/hypothesis_agent.yaml','atlas/config/policy_engine_config.yaml','atlas/config/external_science.yaml','atlas/config/.env.example'):
        p=ROOT/rel
        if p.is_file():selected.add(p)
    test_names={'test_atlas_misuse_guard.py','test_core.py','test_security_guardrails.py','test_episodic_persistence.py','test_semantic_persistence.py','test_web_sensor_async.py','test_integrity_and_bounds.py','test_decision_envelope.py','test_public_content.py','test_sandbox_isolation.py','test_release_http.py','test_model_broker.py','test_science_gates.py','test_runtime_recovery.py','test_goal_focus.py','test_native_session_finish.py','test_experiment_receipt_memory.py','test_execution_evidence.py','test_provenance_epoch.py','test_merkle_tree.py','test_decision_schema.py','test_sandbox_cancellation.py','test_learning_wired.py','test_consolidation_wired.py','test_heartbeat_paper_pipeline.py','test_release_atlas_syntax.py','test_release_privacy.py','test_public_release_hygiene.py','test_cli_config.py','test_installed_atlas_worker.py','test_multimodel_recovery_campaign.py','test_multimodel_recovery_labs.py','test_multimodel_worker_subset.py','conftest.py'}
    selected = {p for p in selected if not unsafe_member(str(p.relative_to(ROOT))) and not nonpublic_member(str(p.relative_to(ROOT)))}
    return sorted(p for p in selected if p.relative_to(ROOT).parts[0]!='tests' or p.name in test_names or p.relative_to(ROOT).as_posix() in POLICY.get('retained_source_files', ()) or 'fixtures' in p.relative_to(ROOT).parts)
...
def main():
    ...
    from scripts.release.curate_manuals import curate
    curate(out, POLICY)
