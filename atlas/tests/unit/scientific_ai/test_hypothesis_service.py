...
def test_hypothesis_service_crud_flow():
    """Test hypothesis persistence service with custom database initialization."""
    # Solo importar lo que necesitamos para evitar conflictos
    from sqlalchemy import create_engine
    from sqlalchemy.orm import sessionmaker
    from app.models.hypothesis_models import HypothesisRecord
    
    # Crear engine y session directamente
    engine = create_engine(os.environ['DATABASE_URL'], echo=False)
    
    # Solo crear tablas de hipótesis
    HypothesisRecord.metadata.create_all(engine)
    
    # Simular la función get_db_session sin importar database_models
    SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    
    # Monkey patch para evitar que el servicio use la inicialización global
    from app import database
    database.SessionLocal = SessionLocal
    
    # Ahora importar y usar el servicio
