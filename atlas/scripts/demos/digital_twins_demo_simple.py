...
class DigitalTwinsDemonstration:
    ...
    async def _demo_optimization_workflows(self):
        ...
        logger.info("⚡ 4. Optimización automática")
    ...
    async def _demo_sensor_synchronization(self):
        ...
        logger.info("📡 5. Sincronización en tiempo real con sensores")
    ...
    async def _demo_lifecycle_management(self):
        ...
        logger.info("🔄 6. Gestión del ciclo de vida")
        ...
        logger.info(f"   🧪 Estado gemelo químico: {chem_status['sync_status']}")
        logger.info(f"      Última sincronización: {chem_status.get('last_sync', 'N/A')}")
        logger.info(f"      Activo: {chem_status.get('is_active', False)}")
        
        logger.info(f"   🦠 Estado gemelo biológico: {bio_status['sync_status']}")
        logger.info(f"      Última sincronización: {bio_status.get('last_sync', 'N/A')}")
        logger.info(f"      Activo: {bio_status.get('is_active', False)}")
    ...
    async def _demo_performance_analytics(self):
        ...
        logger.info(f"   🔮 Predicciones exitosas: {final_stats['successful_predictions']}")
        logger.info(f"   ⚡ Sugerencias de optimización: {final_stats['optimization_suggestions']}")
        logger.info(f"   � Gemelos conectados: {final_stats['connected_twins']}")
        
        # Listar todos los gemelos activos
        active_twins = await self.service.list_digital_twins()
        logger.info("   📋 Lista de gemelos activos:")
        for twin in active_twins:
            logger.info(f"      • {twin['name']} ({twin['id']}) - Tipo: {twin['type']}")
        
        self.results["performance_analytics"] = {
            "final_stats": final_stats,
            "active_twins_list": [
                {"name": twin['name'], "id": twin['id'], "type": twin['type']}
                for twin in active_twins
            ]
        }
