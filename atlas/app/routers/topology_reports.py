...
from app.routers.topology import Point2D
from app.exceptions.domain.biology import BiologyError


class TopologyReportRequest(BaseModel):
    """Request model for generating topology reports.

    This model defines the parameters required for topological analysis,
    including points, epsilon values, and computation settings.
    """
...
class TopologyReportResponse(BaseModel):
    """Response model for topology reports.

    This model contains the results of topological analysis, including
    invariants, persistence data, images, and execution metadata.
    """
    epsilon: float
...
async def topology_report(
    ...
    This endpoint calculates topological invariants and persistence
    for the given points and parameters.
    ...
    Returns:
        TopologyReportResponse with analysis results.
    """
    start = time.time()
    try:
