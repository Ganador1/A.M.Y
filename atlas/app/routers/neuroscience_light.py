...
from app.services.neuroscience_light_service import NeuroscienceLightService
...
from app.types.neuroscience_light_types import (
    FullAnalysisResult,
    ConnectivityAdvancedResult,
    NeuroLightMetricsResult,
    WholeBrainSimulationResult,
    BrainNetworksAnalysisResult,
)
...
async def full_analysis(req: EEGRequest) -> FullAnalysisResult:
    svc = NeuroscienceLightService(sampling_rate_hz=req.sampling_rate_hz)
...
async def connectivity_advanced(req: EEGRequest) -> ConnectivityAdvancedResult:
    svc = NeuroscienceLightService(sampling_rate_hz=req.sampling_rate_hz)
...
@router.get("/metrics")
async def neuro_light_metrics() -> NeuroLightMetricsResult:
...
async def whole_brain_simulation(req: WholeBrainSimulationRequest) -> WholeBrainSimulationResult:
    svc = NeuroscienceLightService()
    return await svc.simulate_whole_brain(req.connectivity_matrix, req.simulation_time_ms)


@router.post("/brain-networks")
async def brain_networks_analysis(req: BrainNetworkAnalysisRequest) -> BrainNetworksAnalysisResult:
    svc = NeuroscienceLightService()
