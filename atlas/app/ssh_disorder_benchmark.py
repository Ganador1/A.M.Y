...
def frontier_metrics(
    ...
    # A near-degenerate doublet admits solver-dependent basis rotations.
    # Its normalized projector diagonal is basis invariant; ordinary mean
    # eigenvector IPR is not. Preserve the preregistered legacy observable and
    # expose the distinct subspace observable for numerical stability audits.
    frontier_probability = np.square(np.abs(eigenvectors[:, frontier_indices]))
    projector_density = frontier_probability.mean(axis=1)
    projector_ipr = float(np.square(projector_density).sum())
    return {
        ...
        "projector_ipr": projector_ipr,
        "normalized_projector_ipr": float(n * projector_ipr),
        "ipr_basis_excess": float(max(0.0, ipr - projector_ipr)),
        # In a connected, zero-onsite even chain the nonzero +/- partners
        # have identical site probabilities. Excess is numerical in this
        # scope; with diagonal disorder it can be physical.
        "chiral_pair_basis_warning": bool(
            np.all(onsite_array == 0.0)
            and np.all(hopping_array > 0.0)
            and ipr - projector_ipr > 1e-8
        ),
    }
                ...
                records.append(
                    {
                        ...
                        "ipr": metrics["ipr"],
                        "normalized_ipr": metrics["normalized_ipr"],
                        "projector_ipr": metrics["projector_ipr"],
                        "normalized_projector_ipr": metrics["normalized_projector_ipr"],
                        "ipr_basis_excess": metrics["ipr_basis_excess"],
                        "chiral_pair_basis_warning": metrics["chiral_pair_basis_warning"],
                        "gap_vote": votes["gap"],
...
def format_benchmark_report(result: dict[str, Any]) -> str:
    ...
    suspect = sum(
        bool(row.get("chiral_pair_basis_warning", False))
        for row in result.get("records", [])
    )
    if suspect:
        lines.append(
            f"  Numerical warning: {suspect} chiral-chain records have "
            "solver-dependent frontier eigenvector IPR; legacy votes are "
            "retained for protocol compatibility, not reliable physical "
            "localization evidence. Inspect projector_ipr and certify small "
            "gaps with ssh_gap_certificate before interpreting a crossover."
        )
    return "\n".join(lines)
