...
class MedicineRegistry:
    ...
    async def register_medical_service(self, service_metadata: ServiceMetadata) -> bool:
        ...
        try:
            ...
            logger.info(f"Medical service registered: {service_metadata.service_id}")
            return True
            
        except MedicalError as e:
            logger.error(f"Failed to register medical service {service_metadata.service_id}: {e}")
            return False
    ...
    async def create_medical_session(self, 
                                   ...
                                   duration_hours: int = None) -> MedicalSession:
        ...
        logger.info(f"Medical session created: {session_id} for patient {patient_id}")
        return session
    ...
    async def create_real_time_stream(self,
        ...
        logger.info(f"Real-time stream created: {stream_id} from {source}")
        return stream

    async def discover_medical_services(self, 
                                      capability: MedicalCapabilityType,
                                      tags: List[str] = None,
                                      patient_context: Dict[str, Any] = None) -> List[ServiceMetadata]:
        ...
        logger.info(f"Found {len(matching_services)} services for capability {capability.value}")
        ...
        for service_config in medical_services:
            ...
            for cap_type in service_config['capabilities']:
                capability = ServiceCapability(
                    ...
                    type=CapabilityType.MEDICAL,  # Usar tipo general, especializado en registry
    ...
    async def _save_state(self):
        ...
        try:
            ...
            with aiofiles.open(self.persistence_path, 'w') as f:
    ...
    async def _load_state(self):
        ...
        try:
            ...
            with aiofiles.aiofiles.open(self.persistence_path, 'r') as f:
                state = json.load(f)
            ...
            logger.info(f"Loaded medicine registry state with {len(sessions_data)} sessions")
...
async def create_medical_session(session_type: str, patient_id: str = None, **kwargs) -> MedicalSession:
