...
from app.domains.mathematics.models import GraphingRequest, GraphResponse


class GraphingService(BaseService):
    """Servicio para generar gráficos matemáticos"""
    
    def __init__(self):
        super().__init__("GraphingService")

    async def process_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Procesa solicitudes de gráficos
        """
        try:
            # Adaptar input dict a GraphingRequest
            if "expression" in request_data:
                # Construir request con valores por defecto si faltan
                request = GraphingRequest(
                    expression=request_data["expression"],
                    variable=request_data.get("variable", "x"),
                    x_min=request_data.get("x_min", -10.0),
                    x_max=request_data.get("x_max", 10.0),
                    points=request_data.get("points", 100),
                    title=request_data.get("title"),
                    x_label=request_data.get("x_label"),
                    y_label=request_data.get("y_label")
                )
                response = self.generate_graph(request)
                return {
                    "success": True,
    ...
    def generate_graph(request: GraphingRequest) -> GraphResponse:
        ...
        try:
            ...
            expression = safe_sympify(request.expression)
    ...
    def generate_multiple_graphs(expressions: List[str], x_min: float = -10, x_max: float = 10, 
        ...
        try:
            ...
            for i, expr_str in enumerate(expressions):
                try:
                    # Parsear expresión
                    expression = safe_sympify(expr_str)
