...
class MaterialsDiscoveryService(BaseService):
    ...
    async def process_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - discover: Discover materials for an application
        - analyze: Analyze a specific material
        """
        operation = request_data.get("operation")
        
        if operation == "discover":
            application = request_data.get("application", "battery_cathode")
            max_candidates = request_data.get("max_candidates", 5)
            candidates = await self.discover_materials_for_application(application, max_candidates)
            return {
                "success": True,
            ...
            if not formula:
                return {"success": False, "error": "Formula required for analysis"}
            # Placeholder for analysis logic
            return {"success": True, "analysis": f"Analysis for {formula}"}
