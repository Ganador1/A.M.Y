...
from app.types.xray_crystallography_service_types import (
    LoadCommonPhasesResult,
...
class XRayCrystallographyService(BaseService):
    ...
    async def process_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process X-ray crystallography requests"""
        ...
        try:
            ...
            if operation == "info":
                return {
                    "common_phases": list(self.common_phases.keys()),
                    "cached_patterns": len(self.diffraction_cache)
                }
                
            elif operation == "simulate_pattern":
                structure = request_data.get("structure", {})
                wavelength = request_data.get("wavelength", 1.5406)
                result = await self.simulate_diffraction_pattern(structure, wavelength)
                return result.dict()
                
            elif operation == "solve_phase":
                diffraction_data = request_data.get("diffraction_data", {})
                result = await self.solve_phase_problem(diffraction_data)
                return result.dict()
                
            elif operation == "refine_structure":
                initial_structure = request_data.get("initial_structure", {})
                diffraction_data = request_data.get("diffraction_data", {})
                result = await self.refine_structure(initial_structure, diffraction_data)
                return result.dict()
                
            elif operation == "powder_analysis":
                powder_data = request_data.get("powder_data", {})
                result = await self.powder_diffraction_analysis(powder_data)
                return result
    ...
    def _load_common_phases(self) -> LoadCommonPhasesResult:
